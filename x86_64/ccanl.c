...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 12, "cvodeieq");
 	...
 	ivoc_help("help ?1 ccanl /home/dhh/soltesz-lab/losonczy-ca3-collab/v2-3232021/x86_64/ccanl.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dncai = - ( inca ) / depth / 96520.0 * ( 1e7 ) + ( caiinf / 3.0 - ncai ) / catau ;
   Dlcai = - ( ilca ) / depth / 96520.0 * ( 1e7 ) + ( caiinf / 3.0 - lcai ) / catau ;
   Dtcai = - ( itca ) / depth / 96520.0 * ( 1e7 ) + ( caiinf / 3.0 - tcai ) / catau ;
   cai = ncai + lcai + tcai ;
 ...
 /*END CVODE*/
 static int integrate (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    ncai = ncai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / catau)))*(- ( ( ( ( - ( inca ) ) / depth ) / 96520.0 )*( ( 1e7 ) ) + ( ( ( caiinf ) / 3.0 ) ) / catau ) / ( ( ( ( - 1.0 ) ) ) / catau ) - ncai) ;
    lcai = lcai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / catau)))*(- ( ( ( ( - ( ilca ) ) / depth ) / 96520.0 )*( ( 1e7 ) ) + ( ( ( caiinf ) / 3.0 ) ) / catau ) / ( ( ( ( - 1.0 ) ) ) / catau ) - lcai) ;
    tcai = tcai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / catau)))*(- ( ( ( ( - ( itca ) ) / depth ) / 96520.0 )*( ( 1e7 ) ) + ( ( ( caiinf ) / 3.0 ) ) / catau ) / ( ( ( ( - 1.0 ) ) ) / catau ) - tcai) ;
   cai = ncai + lcai + tcai ;
   eca = ktf ( _threadargs_ ) * log ( cao / cai ) ;
   enca = eca ;
   elca = eca ;
...
double ktf ( _threadargsproto_ ) {
   double _lktf;
 _lktf = ( 1000.0 ) * 8.3134 * ( celsius + 273.15 ) / ( 2.0 * 96520.0 ) ;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/dhh/soltesz-lab/losonczy-ca3-collab/v2-3232021/ccanl.mod";
static const char* nmodl_file_text = 
  "COMMENT\n"
