...
#     tt    = np.asarray(list(expanded_t) + list(tt), dtype='float32')
    ...
    return np.asarray(spike_times, dtype='float32') + delay
...
class Arena(object):
    ...
    def generate_spike_times(self, population, dt=0.05, delay=0, cued=False):
        ...
        if cued:
            self.cued_positions = np.linspace(25., self.arena_size-25., np.sum(is_spatial))
        for (gid, fr) in enumerate(firing_rates):
            ...
            for n in range(nlaps):
                if not is_spatial[n]:
                    ...
                    if cued:
                        random_position = self.cued_positions[online_number]
...
class WiringDiagram(object):
                ...
                try:
                    ...
                    if popA_id == 0 and popB_id == 0:
                        am = self.create_adjacency_matrix(popA_pos, popB_pos, nA, nB, convergence, self.internal_con_rnd, 
                                                          inv_func='exp', same_pop=same_pop)
                    else:
    ...
    def generate_external_connectivity(self, external_information, **kwargs):
        ...
        for src_id,src_pop in list(zip(external_ids,external_pops)):
            ...
            for dst_pop in self.pops:
                ...
                if dst_pop_id == 0 and src_id < 102: # For MF and MEC connections
                    am = self.create_adjacency_matrix(src_pos, dst_pos, nsrc, ndst, convergence, self.external_con_rnd, 
                                                      inv_func='inv', valid_gids=dst_gids_to_connect_to)
                else:
    ...
    def create_adjacency_matrix(self, src_coordinates, dst_coordinates, nsrc, ndst, convergence, rnd, 
                                inv_func=None, same_pop=False, valid_gids=None):
        ...
        for d in range(ndst):
            ...
            if src_coordinates is not None and dst_coordinates is not None:
                ...
                if inv_func == 'inv':
                    inv_dist  = 1./(distances+1.0e-5)**2
                    #inv_dist = np.exp(-distances/0.01)
                elif inv_func == 'exp':
                    inv_dist = np.exp(-distances/0.01)
                if same_pop: inv_dist[d] = 0.0                
#                 if d == 75: 
#                     plt.figure()
#                     plt.plot(inv_dist/np.sum(inv_dist))
#                     plt.show()
                pcon      = inv_dist/np.sum(inv_dist)
                presynaptic_gids = rnd.choice(np.arange(nsrc), p=pcon, replace=False, size=(convergence,))
                
            else:
                presynaptic_gids = rnd.choice(np.arange(nsrc), replace=True, size=(convergence,))
